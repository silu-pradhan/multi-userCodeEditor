...
function CopilotView() {
    ...
    return (
        <div
            className="flex max-h-full min-h-[400px] w-full flex-col gap-2 overflow-hidden bg-dark p-4"
            style={{ height: viewHeight }}
        >
            <h1 className="view-title">Copilot</h1>
            <textarea
                className="min-h-[120px] w-full rounded-md border border-darkHover bg-darkHover p-2 text-white outline-none transition-colors focus:border-primary focus:ring-1 focus:ring-primary resize-none"
            ...
            <div className="flex-grow rounded-lg w-full overflow-y-auto border border-darkHover bg-darkHover p-2\">
                <ReactMarkdown
                    components={{
                        ...
                        code({ inline, className, children,  }: any) {
                            ...
                            return !inline ? (
                                <SyntaxHighlighter
                                    ...
                                    language={language}
                                    PreTag="pre"
                                    className="!m-0 !rounded-md !bg-gray-900 !p-2 !text-xs"
                                >
                                    {String(children).replace(/\n$/, "")}
                                </SyntaxHighlighter>
                            ) : (
                                <code className="bg-darkHover px-1 py-0.5 rounded text-sm\">
                                    {children}
                                </code>
                            )
                        },
